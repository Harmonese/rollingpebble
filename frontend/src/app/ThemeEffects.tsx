...
import { ThemeMode } from "../shared/preferences.js";
import { SETTINGS_UPDATED_EVENT } from "../shared/settingsEvents.js";
import { workspaceBgStatus } from "../shared/api/settings.js";
...
export const ThemeEffects: React.FC = () => {
    const { prefState } = useContext(appContext, AppContextBits.prefState);
    const [bgVersion, setBgVersion] = useState(Date.now());
    const [bgSource, setBgSource] = useState<"custom" | "default">("default");
    ...
    useEffect(() => {
        ...
        const imageUrl = bgSource === "custom"
            ? `/api/settings/workspace-bg?v=${bgVersion}`
            : "/img/rollingpebble-workspace-bg.webp";
        el.textContent = `.studio-editor-host { background: radial-gradient(circle at 58% 46%, rgba(var(--theme-rgb), 0.08), transparent 38%), var(--editor-bg-overlay), url(${imageUrl}), var(--studio-bg); background-repeat: no-repeat, no-repeat, no-repeat, repeat; background-position: center center, center center, center center, center center; background-size: auto, auto, cover, auto; }`;
        return () => {
    ...
    useEffect(() => {
        const refreshBackground = async () => {
            try {
                const status = await workspaceBgStatus();
                setBgSource(status.source === "custom" ? "custom" : "default");
            } catch {
                setBgSource("default");
            } finally {
        ...
        const handler = () => void refreshBackground();
